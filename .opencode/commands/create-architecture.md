...
11. **Self-Validation**
    After generating documentation, validate the output contains expected sections:
    ```
    uv run .opencode/hooks/validators/file_validator.py --directory docs --extension .md --modified-within 300 --contains "## Architecture" --contains "## Components"
    ```
    If validation fails, fix the documentation before reporting completion.
