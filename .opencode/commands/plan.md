---
description: Creates a concise engineering implementation plan based on user requirements and saves it to .opencode/plans directory
...
Create a set of TD tasks that map 1:1 (or close) to the implementation steps. Include enough detail that the tasks can be created immediately.

For each TD task, include:
- Title
- Type (task|feature|bug|chore|docs)
- Priority (P0|P1|P2|P3)
- Acceptance Criteria
- Notes about file ownership or sequencing (if it depends on another task)

Also include the exact TD CLI commands needed to create them (and set parent relationships when applicable), for example:

```bash
td create "<title>" --type task --priority P2 --description "<desc>"
td start <id>
```
