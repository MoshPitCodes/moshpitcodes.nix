...
### 5. Session Context Plugin (`session-context.ts`)

**Purpose**: Tracks session statistics and git repository context.

**Features**:
- Tracks git branch and uncommitted changes on session start
- Counts tools used and files modified during session
- Tracks unique bash commands executed
- Calculates session duration
- Logs session stats to `logs/session.jsonl`
- Stores detailed history in `.opencode/data/sessions/{session-id}.json`
- Prints session summary on completion

**Hooks**: `"tool.execute.after"` (typed), `event` (generic, filters `session.created`, `session.idle`, `session.error`, `file.edited`)
...
**Log Output** (`logs/session.jsonl`):
```json
{"event":"session_started","sessionId":"abc123","timestamp":"2026-02-09T12:34:56.789Z","git":{"branch":"main","uncommittedChanges":true,"changedFiles":3}}
{"event":"session_ended","sessionId":"abc123","timestamp":"2026-02-09T12:50:38.456Z","duration":942,"stats":{"toolsUsed":37,"filesModified":8,"commands":["npm","git","bun","mkdir"]},"git":{"branch":"main","uncommittedChanges":true,"changedFiles":11}}
```

**Session Data File** (`.opencode/data/sessions/{session-id}.json`):
```json
{
  "sessionId": "abc123",
  "startTime": "2026-02-09T12:34:56.789Z",
  "endTime": "2026-02-09T12:50:38.456Z",
  "duration": 942,
  "git": {
    "branch": "main",
...
### 6. Notifications Plugin (`notifications.ts`)

**Purpose**: Sends system notifications for session events.

**Features**:
- Sends notification on session completion
- Sends notification on session errors
- Cross-platform support:
  - **macOS**: Uses `osascript` (AppleScript) - always available
  - **Linux**: Uses `notify-send` (libnotify) - requires DISPLAY/WAYLAND_DISPLAY
  - **Windows**: Uses PowerShell Toast notifications - always available
- Automatically detects display environment availability
- Silently skips notifications in headless/SSH/WSL environments (no console clutter)

**Hook**: `event` (generic), filters `session.idle` and `session.error`

**Requirements**:
- **macOS**: No additional requirements (built-in notification center)
- **Linux**: 
  - `notify-send` installed (`sudo apt install libnotify-bin` on Debian/Ubuntu)
  - Display server running (X11 or Wayland)
  - `DISPLAY` or `WAYLAND_DISPLAY` environment variable set
- **Windows**: No additional requirements (built-in notification center)

**Notification Examples**:

**Session Completed** (macOS):
```
✅ OpenCode Session Completed
Session abc12345 has finished.
```

**Session Error** (Linux):
```
⚠️ OpenCode Session Error
Session abc12345 encountered an error: Connection timeout
```

**Headless/SSH Environment**: Notifications are silently skipped (no output)
...
Within the project plugin directory, files are loaded alphabetically. This project prefixes plugin filenames to ensure correct ordering (security first, notifications last).
...
└── data/
    └── sessions/                    # Session data storage
        ├── {session-id}.json        # Session context
        └── {session-id}-snapshot.json  # Filesystem snapshot
```
...
5. **Notifications**: Verify system notification appears on session end
...
**Expected Behavior**: 
- In headless/SSH/WSL environments without a display server, notifications are **silently skipped** (by design)
- No console fallback messages are shown to avoid clutter

**Check** (if you expect notifications to work):
1. **Linux**: Verify display environment
   ```bash
   echo $DISPLAY          # Should output something like ":0" or ":1"
   echo $WAYLAND_DISPLAY  # Or something like "wayland-0"
   ```
2. **Linux**: Verify `notify-send` is installed
   ```bash
   which notify-send      # Should output path like /usr/bin/notify-send
   sudo apt install libnotify-bin  # Install if missing (Debian/Ubuntu)
   ```
3. **Linux**: Test D-Bus notification service
   ```bash
   notify-send "OpenCode" "Test"  # Should show desktop notification
   # If error: "ServiceUnknown: The name org.freedesktop.Notifications..."
   # → You're in a headless environment (notifications will be skipped)
   ```
4. **macOS**: No additional setup needed (built-in notification center)
5. **Windows**: No additional setup needed (built-in notification center)
...
# Linux (requires display environment)
notify-send "OpenCode" "Test"

# Windows
powershell -Command "[Windows.UI.Notifications.ToastNotificationManager]::Show(...)"
```
