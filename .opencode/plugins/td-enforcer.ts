...
export const TDEnforcerPlugin: Plugin = async ({ directory, client, $ }) => {
  const enabled = process.env.OPENCODE_ENABLE_TD_ENFORCER === "1"
  if (!enabled) {
    console.warn(
      "TD enforcer plugin is disabled (set OPENCODE_ENABLE_TD_ENFORCER=1 to enable).",
    )
    return {}
  }
  ...
  /**
   * Start watching `.todos/db.sqlite` for changes.
      ...
      switch (event.type) {
        ...
        case "session.idle": {
          ...
          if (
            ...
            console.log(
              "\nTip: Run 'td handoff' to capture working state before ending your session.\n",
            )
          }
