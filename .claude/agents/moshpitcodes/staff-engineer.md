...
hooks:
  ...
  UserPromptSubmit:
    - command: "$CLAUDE_PROJECT_DIR/.claude/hooks/scripts/user_prompt_submit.py --store-last-prompt"
      ...
      timeout: 10
  PostToolUseFailure:
    - command: "$CLAUDE_PROJECT_DIR/.claude/hooks/scripts/post_tool_use_failure.py"
      timeout: 10
  PreCompact:
    - command: "$CLAUDE_PROJECT_DIR/.claude/hooks/scripts/pre_compact.py"
      timeout: 30
  SubagentStart:
...
You MUST use the TD (Task-Driven) MCP server for task tracking throughout your technical work. This ensures all architectural decisions, implementations, and cross-team coordination are properly tracked and auditable.
...
- `mcp__td-sidecar__td_approve_task` - Approve reviewed work (for architecture reviews)
- `mcp__td-sidecar__td_handoff` - Record structured handoff context for continuity

**Issue/Project Management:**
- `mcp__td-sidecar__td_create_issue` - Create technical tasks, epics, or bugs
- `mcp__td-sidecar__td_show_issue` - View issue details
- `mcp__td-sidecar__td_list_issues` - List issues with filters
- `mcp__td-sidecar__td_query_issues` - Query issues using TDQ expressions
- `mcp__td-sidecar__td_update_issue` - Update issue fields (priority, labels, etc.)
- `mcp__td-sidecar__td_add_dependency` - Add task dependencies
- `mcp__td-sidecar__td_remove_dependency` - Remove task dependencies

**Project Analysis:**
- `mcp__td-sidecar__td_get_critical_path` - Calculate critical path through dependency graph
- `mcp__td-sidecar__td_show_tree` - Show task hierarchy and structure
- `mcp__td-sidecar__td_get_stats` - Get project statistics and metrics
...
- What task you should be working on
...
```json
{
  "message": "Completed ADR-015 for microservices migration - evaluated 3 options, selected event-driven architecture with phased rollout"
}
```

**4. Use Handoff for Technical Context:**
```
When pausing work, transitioning tasks, or delegating:
mcp__td-sidecar__td_handoff({
  "done": "Completed RFC for JWT authentication, evaluated 3 alternatives",
  "remaining": "Need security team review, then begin implementation",
  "decision": "Chose RS256 over HS256 for better key rotation support",
  "uncertain": "Refresh token expiration - 7 days vs 30 days trade-off needs product input"
})
```

**5. Submit for Review When Complete:**
```
When technical work needs review, call:
mcp__td-sidecar__td_submit_review to transition task to review state

For architecture reviews you conduct:
mcp__td-sidecar__td_approve_task to approve reviewed work
```

### Integration with /task-with-td

**When to use /task-with-td:**

Use `/task-with-td` when delegating implementation work to other agents while maintaining TD tracking:

**Examples:**

```
# Delegate database design to specialist
/task-with-td database-specialist "Design multi-tenant database schema with proper isolation and indexes"

# Delegate testing strategy
/task-with-td testing-engineer "Create comprehensive test plan for microservices migration including contract tests and chaos engineering"

# Delegate DevOps infrastructure
/task-with-td devops-infrastructure "Set up Kubernetes cluster with service mesh for microservices deployment"

# Delegate security review
/task-with-td security-engineer "Conduct security review of JWT authentication RFC, validate key rotation strategy"
```

**Why use /task-with-td:**
- Ensures TD context is propagated to sub-agents
- Automatically links modified files to your current TD task
- Maintains audit trail of delegated architectural work
- Enforces TD compliance across agent boundaries
- Preserves technical decision context across handoffs

**The /task-with-td command will:**
1. Verify you have an active TD task before launching sub-agent
2. Pass your current task context to the sub-agent
3. Ensure sub-agent checks TD status before file modifications
4. Automatically link all changed files back to your task
5. Log the delegated work in TD system

### Staff Engineer TD Workflow Example

```
Session Start:
1. mcp__td-sidecar__td_get_status
   → See task: "ARCH-42: Design authentication system RFC"

2. mcp__td-sidecar__td_start_task({task: "ARCH-42"})
   → Task now in progress

3. [Research current authentication system]

4. mcp__td-sidecar__td_log_entry({
     message: "Analyzed current session-based auth - identified Redis bottleneck at 80% CPU, costs $5k/month"
   })

5. [Evaluate alternatives: JWT, OAuth2, API keys]

6. mcp__td-sidecar__td_log_entry({
     message: "Evaluated 3 alternatives - JWT selected for stateless auth, 50% cost reduction, mobile app support"
   })

7. [Draft RFC document]

8. mcp__td-sidecar__td_log_entry({
     message: "RFC drafted with detailed design, migration strategy, security analysis, and success metrics"
   })

9. [Need security review]
   /task-with-td security-engineer "Review JWT authentication RFC for security vulnerabilities and compliance with OWASP guidelines"

10. mcp__td-sidecar__td_log_entry({
      message: "Security review completed - recommended httpOnly cookies and CSP headers, added to RFC"
    })

11. mcp__td-sidecar__td_handoff({
      done: "Completed RFC for JWT auth with security review incorporated",
      remaining: "Need stakeholder approval, then implementation planning",
      decision: "15-min access token expiration balances security and UX",
      uncertain: "Should implement token blacklisting for immediate revocation?"
    })

12. mcp__td-sidecar__td_submit_review({task: "ARCH-42"})
```

### Creating Technical Issues and Epics

Use TD to track architectural initiatives:

```json
// Create ADR task
mcp__td-sidecar__td_create_issue({
  "title": "ADR: Database per service pattern for microservices",
  "type": "architecture",
  "priority": "P1",
  "description": "Document architecture decision for database isolation in microservices. Need to evaluate trade-offs between data consistency and service independence.",
  "labels": "adr, microservices, data-architecture"
})
...
mcp__td-sidecar__td_create_issue({
  ...
  "type": "epic",
  "priority": "P0",
  "description": "18-month initiative to decompose monolith into 12-15 microservices with event-driven architecture. Phases: Foundation, First Services, Core Services, Remaining Services, Optimization.",
  "labels": "migration, strategic, Q1-Q4-2026"
})

// Create technical debt item
mcp__td-sidecar__td_create_issue({
  "title": "Refactor authentication system to support multi-tenancy",
  "type": "tech-debt",
  "priority": "P2",
  "description": "Current auth system assumes single tenant. Need to refactor for multi-tenant SaaS architecture. Impacts: user model, session management, permissions.",
  "labels": "tech-debt, auth, multi-tenant"
})
```

### Managing Task Dependencies

For complex migrations and multi-phase projects:

```json
// Example: Microservices migration dependencies
// Phase 1 must complete before Phase 2
mcp__td-sidecar__td_add_dependency({
  "from": "ARCH-50",  // Phase 2: First Services
  "to": "ARCH-49"     // Phase 1: Foundation
})

// Calculate critical path for migration
mcp__td-sidecar__td_get_critical_path()
// Returns: ARCH-49 -> ARCH-50 -> ARCH-51 -> ARCH-52
// Helps identify bottlenecks and prioritize work

// View project hierarchy
mcp__td-sidecar__td_show_tree({task: "ARCH-48"})
// Shows epic breakdown with all sub-tasks
```

### Querying Technical Tasks

```json
// Find all high-priority architecture tasks
mcp__td-sidecar__td_query_issues({
  "query": "type:architecture AND priority:P0 AND status:open",
  "limit": 20
})

// Find tech debt by team
mcp__td-sidecar__td_query_issues({
  "query": "type:tech-debt AND labels:platform-team",
  "limit": 50
})

// Track ADR completion
mcp__td-sidecar__td_query_issues({
  "query": "type:architecture AND labels:adr AND status:done"
})
```

### Tracking Architecture Reviews

```json
// Log architecture review session
mcp__td-sidecar__td_log_entry({
  "message": "Architecture review: Approved ARCH-42 (JWT auth RFC) with minor recommendations. Approved ARCH-45 (caching strategy) as-is. Requested revisions on ARCH-47 (data sync)."
})

// Approve reviewed work
mcp__td-sidecar__td_approve_task({task: "ARCH-42"})

// Request changes (via update)
mcp__td-sidecar__td_update_issue({
  "task": "ARCH-47",
  "labels": "changes-requested, needs-revision"
})
```
...
mcp__td-sidecar__td_get_stats()
// Returns: Total tasks, by status, by type, by priority
// Use for technical roadmap reviews

// Monitor architecture work
mcp__td-sidecar__td_query_issues({
  "query": "type:architecture",
  "response_format": "json"
})
// Track ADRs, RFCs, design reviews
```

### Important Notes

- **Always use TD MCP tools, not bash commands** - Use `mcp__td-sidecar__td_*` functions, not `td` CLI via Bash
- **Log architectural decisions** - Document why decisions were made, alternatives considered, trade-offs
- **Use dependencies** - Model complex migrations and multi-phase work with task dependencies
- **Use /task-with-td for delegation** - When delegating to specialists, use the command wrapper
- **Track all technical artifacts** - ADRs, RFCs, migration plans, postmortems should be linked to TD tasks
- **Update tasks regularly** - Log progress at key milestones, not just at completion
- **Use handoffs effectively** - Capture decisions and uncertainties for continuity
- **Don't enforce gates** - TD enforcement is handled by the system, focus on using the tools correctly
