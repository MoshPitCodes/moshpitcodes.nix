...
"""
Status Line v11 - Combined Workspace + Resource Tracking + MCP Servers
Display: [Model] | 📁 directory | 🌿 branch ±N | 🔌 mcp-server1, mcp-server2 | [###---] 42.5% | ~115k left
Adds MCP server detection to v10
"""
...
def get_active_mcp_servers():
    """Detect active MCP servers from running processes."""
    mcp_servers = []
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for line in lines:
                ...
                if 'mcp' in line.lower() and ('server' in line.lower() or '.js' in line or 'dist/index' in line):
                    # Extract server name from path
                    # Example: /path/to/td-sidecar-mcp-server-1.0.0/lib/td-sidecar-mcp/dist/index.js
                    if 'td-sidecar' in line:
                        if 'td-sidecar' not in mcp_servers:
                            mcp_servers.append('td-sidecar')
                    elif 'stripe' in line.lower():
                        if 'stripe' not in mcp_servers:
                            mcp_servers.append('stripe')
...
def generate_status_line(input_data):
    """Generate the combined status line with workspace, MCP servers, and resource tracking."""
    parts = []
    ...
    if mcp_servers:
        ...
        parts.append(f"{MAGENTA}🔌 {mcp_list}{RESET}")
